...
from .contracts import (BASS_WAV, BEATS_JSON, META, NOTES_JSON, PITCH_NPZ, TAB_JSON, TAB_TEX,
                        PipelineError, Tab, load_beats, load_meta, load_notes, save_json)
...
def run(source: str, job_dir: Path, sep_model: str, device: str, force: bool) -> Path:
    steps = [
    ...
    tab = Tab(meta.title, beats.bpm, beats.beats_per_bar,
              stage5_frets.assign(load_notes(job_dir / NOTES_JSON)))
...
def main() -> None:
    ...
    try:
        out = run(a.source, job_dir, a.sep_model, a.device, a.force)
